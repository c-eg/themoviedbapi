...
public class TmdbAuthentication extends AbstractTmdbApi {
     ...
     * 
     * @param tokenin A TokenAuthirisation previously generated by getAuthorisationToken
     * @param user username
     * @param pwd password
     * @return The validated TokenAuthorisation. The same as input with getSuccess()==true
     */
    
    public TokenAuthorisation getLoginToken(TokenAuthorisation tokenin,String user, String pwd) {
        ApiUrl apiUrl = new ApiUrl(TMDB_METHOD_AUTH, "token/validate_with_login");
        apiUrl.addParam("request_token", tokenin.getRequestToken());
    ...
    public TokenSession getSessionLogin(String username,String passwd)  {
    	TokenAuthorisation token_in = getAuthorisationToken();
    	
    	if(token_in == null || !token_in.getSuccess()) {
	        logger.warn("Authorisation token was not successful!");
	        throw new MovieDbException(MovieDbExceptionType.AUTHORISATION_FAILURE, "Authorisation token was not successful!");
    	}
    	
    	TokenAuthorisation token_out = getLoginToken(token_in,username,passwd);

    	if(token_out == null || !token_out.getSuccess()) {
	        logger.warn("Login token was not successful!");
	        throw new MovieDbException(MovieDbExceptionType.AUTHORISATION_FAILURE, (token_out == null ? "" : token_out.getStatusMessage()));
    	}
    	
    	return getSessionToken( token_out );
    }
